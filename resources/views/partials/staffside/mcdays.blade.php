                ...
                <div class="card-body p-5 text-center">
                    <div class="numbers">
                        @php
                            ...
                            $totalDays = Auth::user()->$columnName ?? 0; // Total days set by the admin
                            $remainingDays = old($columnName, $totalDays); // Remaining days (dynamically updated)
                        @endphp
                        <p class="text-md mb-0 text-uppercase font-weight-bold">{{ $note->title }}</p>
                        <h5 class="font-weight-bolder">
                            Baki cuti <br>
                            {{ $remainingDays }} Hari / {{ $totalDays }} Hari
                        </h5>
                    </div>
